...
이제서야 제대로된 원인에 대해 작성한다. 원래 최초 작성일로부터 하루 뒤에 원인을 팀원덕분에 알게 되었지만 업데이트를 못했다 🥹

결론부터 얘기하자면 해당 문제는 Docker로 RabbitMQ를 실행하니 바로 해결됐다. 한마디로 정의하긴 애매하지만 어쨌든 로컬에서 직접 실행하면 될 때도 있고 안될 때도 있었는데 docker로 실행하니 훨씬 안정적이었다. 
